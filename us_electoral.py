...
from state_data_2020 import electoral_college  # Pull in Electoral College Data
...
def ec_result(state_results):
    ...
    ec = electoral_college  # Make a copy of the state results
    results = {}
    for state in state_results.keys():
        winner = state_results[state]
        if winner in results.keys():
            results[winner] += electoral_college[state]
        else:
            results[winner] = electoral_college[state]
...
for state in electoral_college:
    ...
    while redo:
        ...
        # TODO: Add Handler for special case states
